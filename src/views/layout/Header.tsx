...
import {
  FaFacebookF, FaTwitter, FaSkype, FaGoogle,
  ...
  return (
    <header>
      <div className="bg-[#243559] py-1.5">
        <div className="max-w-[1170px] mx-auto text-sm px-6 py-2 flex flex-wrap justify-center sm:justify-between items-center gap-2 sm:gap-4">
          <div className="flex flex-wrap gap-4 text-xs md:text-sm">
            <div className="flex items-center gap-1 text-[#CCCCCC] cursor-pointer">
              <IoMdMail className="text-white" /> info@randerc3.com
            </div>
            <div className="flex items-center gap-1 text-[#CCCCCC] cursor-pointer">
              <FaPhoneSquareAlt className="text-white" /> +3234-7665545
            </div>
            <div className="flex items-center gap-1 text-[#CCCCCC] cursor-pointer">
          ...
          <div className="flex gap-2">
            {[FaSkype, FaTwitter, FaFacebookF, FaGoogle].map((Icon, i) => (
      ...
      <div className="px-6 py-4 shadow-lg bg-[#FFFFFFCC] relative z-50">
        <div className="max-w-[1170px] mx-auto flex justify-between items-center">
              ...
              {menuItems.map((item, index) => (
                ...
                >
                  <div className="flex items-center gap-1">
                    <Link href={item.path} className="text-[#444444] hover:text-primary text-sm lg:text-base font-medium flex flex-col items-start">{item.label}</Link>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <Buttons variant="default">It Consultant</Buttons>
          </div>
      ...
      <div className={`fixed top-0 left-0 h-full w-[250px] bg-white shadow-lg z-[1000] p-5 transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-6 flex gap-4 items-center">
          <button className="text-black text-lg"><FaSearch /></button>
          <Buttons variant="default" className="">It Consultant</Buttons>
        </div>
      </div>
    </header>
  );
